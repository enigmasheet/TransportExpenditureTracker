...
public class VatCalculationMatchAttribute : ValidationAttribute
{
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        {
            var rowErrors = ValidateRow(details[index]);
    ...
    private static List<string> ValidateRow(ExpenseDetailViewModel d)
    {
        var rowErrors = new List<string>();

        if (d.Quantity > 0 && d.Rate > 0)
        {
            var computedTaxable = Math.Round(d.Quantity * d.Rate, 2, MidpointRounding.AwayFromZero);
